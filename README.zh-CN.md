...
它不是泛泛的 agent framework，也不是 agent 群聊。项目负责人把目标交给 PM Agent；PM 拆任务并写验收标准；Dev、Test、Review、Version、Research、UX 等角色各自执行；`team-loop/progress.md` 持续记录项目状态；所有消息、决策和提交记录都落在仓库本地。
...
- **项目状态不漂移**：`team-loop/progress.md` 是 PM 维护的项目进度文件和单一事实源。
- **repo-local 审计记录**：`messages.ndjson`、`decisions.ndjson`、`commits.ndjson` 记录派发、审批和版本动作。
- **真实工程安全边界**：安装第三方技能、删/合并分支、重写公开历史、正式发布都需要 Dylan 确认。
...
这会创建项目本地的 `team-loop/` 工作区，用于 Agent roster、消息日志、决策日志、提交日志、角色 profile 和 `progress.md`。
...
## `team-loop/progress.md`

`team-loop/progress.md` 是 PM Agent 维护的 Project Progress File，也是多 Agent 协作的单一事实源。
